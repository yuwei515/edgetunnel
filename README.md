...
订阅地址支持按 **地区筛选节点**（内置 Cloudflare IP 段到地区映射，自动识别）：

- 节点命名：用户在 `ADD.txt` 里未带 `#备注` 的 CF 优选 IP，自动按 IP 段识别为「地区-序号」形如 `香港-1`、`日本-2`；带 `#备注` 则优先用用户备注。
- `region` 查询参数筛选（Clash/Base64/Sing-box 等订阅均支持，逗号分隔多地区）：
   ```url
   /sub?token=xxx&region=香港
   /sub?token=xxx&region=美国,日本
   ```
   仅返回命中地区的节点；未命中地区的 IP（如非 CF 段或无法识别）不在筛选结果内。
